...
### In-class discussion & interaction

- Please join Slido using:
  - Sign-in using the [link:](https://app.sli.do/event/5EqCfteDXkd4XLL2BKZy4a)
  - QR code:  
  - <img src="images/Slido.png" width="200px">

:::

::: {.column width="50%"}

### Post-class discussion board {.fragment}

- Please join Padlet using:

  - [Link to the question](https://padlet.com/ismayillimehman/ml-discussion-board-sau21g08d8fn5sr2/wish/Ae2Ravz9A1D0anz4)
  - QR code: 
  - <img src="images/padletQR_code.png" width="220px">
...
<iframe src="https://app.sli.do/event/5EqCfteDXkd4XLL2BKZy4a" height="100%" width="100%" frameBorder="0" style="min-height: 560px;" allow="clipboard-write" title="Slido"></iframe>
...
- By the end of this session, you should be able to:
  ...
  - Understand the role of the **log-likelihood function** in MLE.
...
<div style="background-color:#f0f8ff; padding: 0.5em; border-radius: 2px;">
  ...
  <ul>
    <li class="fragment">Learned key ideas in <strong>probability theory</strong>, assuming we know the full distribution (pmf/pdf):
      <ul>
        <li class="fragment">How to compute <strong>moments</strong> of a random variable</li>
        <li class="fragment">How to find <strong>cdfs</strong> and <strong>interval probabilities</strong></li>
        <li class="fragment">How to define and verify <strong>independence</strong></li>
      </ul>
  <li class="fragment">✔️ <strong>Assumption</strong>: We know the distribution and its parameters.</li>
  </ul>
</div>

<div class="fragment" style="background-color:#f2f2f2; padding: 0.5em; border-radius: 5px; margin-top: 0.5em;">
  <h3 class="fragment"><strong>Now, Let’s Get Real</strong></h3>
  <ul>
    <li class="fragment" style="color: red;"><strong>In practice, we rarely know the actual distribution or its parameters!</strong></li>
    <li class="fragment">What we <strong>do</strong> have is <strong>data</strong> — observations from the unknown distribution.</li>
    <li class="fragment">Using this data, we can:
      <ul>
        <li class="fragment"><strong>Infer</strong> characteristics of the distribution</li>
        <li class="fragment"><strong>Estimate</strong> unknown quantities (like parameters or moments)</li>
      </ul>
  <li class="fragment">🔍 <strong>Key idea</strong>: Use data to learn what we don’t know.</li>
  </ul>
</div>
...
- Frequentist methods & Bayesian estimators

<blockquote class="fragment" style="text-align: center; margin-top: 2em;">
    No single method is uniformly superior
</blockquote>

<div class="fragment" style="text-align: center; margin-top: 2em;">

### <span style="font-size: 1.5em; font-weight: 600; color: #1e3a8a;">
...
<div class="fragment" style="background-color:#f0f8ff; padding: 1em; border-radius: 5px; margin-top: 0.5em;">

### Setting

- The underlying distribution of data is *(supposedly)* known
- However, the parameters of that distribution are *unknown*
- Goal: Estimate **parameters** of that distribution

</div>

<div class="fragment" style="background-color:#f2f2f2; padding: 1em; border-radius: 5px; margin-top: 0.5em;">

### Example

- A certain random variable has distribution of $N(\mu, \sigma^{2})$ but **not known**  mean and variance parameters.
- Estimate the *mean* and *variance*
- Maximum likelihood method will estimate values of $\mu$ and $\sigma^{2}$ that make our observed data most likely
</div>
---
...
<div>
  ...
  <input type="range" id="mu" min="0" max="10" step="0.01" value="0" oninput="updatePlot()">
  <span id="muVal">5</span>
</div>
<div>
  <label for="sigma">Standard Deviation (σ):</label>
  <input type="range" id="sigma" min="0.1" max="5" step="0.01" value="0" oninput="updatePlot()">
  <span id="sigmaVal">2</span>
</div>
...
- $\hat{\mu} = 5.21$ & $\hat{\sigma} = 1.82$

- [Detailed Jupyter Notebook here](https://colab.research.google.com/drive/1RjP9kjG-q3UhCsT4Fn7dpNvm-d_huYxa?usp=sharing) 
...
- Observe $n$ data points, $Y_{1}, Y_{2}, \dots, Y_{n}$.
  ...
  - The joint *pdf* of $\mathbf{Y}$ is given by $f(\mathbf{Y}; \theta)$
    - $\theta$ contains the parameters of this pdf and can be a *vector* containing multiple parameters
</div>

<div class="fragment" style="background-color:#fff9e5; padding: 0.5em; border-radius: 2px; margin-top: 0em;">

### Problem

- <span style="color: red;">**We do NOT know what $\theta$ is.**</span>
...
- The likelihood function is simply equivalent to the probability density function (pdf): $$L(\theta; \mathbf{Y})= f(\mathbf{Y}; \theta)$$. 
  - This is called the *joint likelihood function*, since it is equivalent to the *joint pdf* of $\mathbf{Y}$.
</div>

- The maximum likelihood estimator of $\theta$ is 
$$\hat{\theta} = \arg \max_{\theta} L(\theta; \mathbf{Y}) $$.

  - ML is based on the idea that $\hat{\theta}$ should be the value of ${\theta}$ which maximises the likelihood of observing whatever dataset we have at hand

  - In other words, we are trying to find the value of $\theta$ which makes the observed data as likely as possible to have been observed.
...
<strong> When data are independently and identically distributed, the joint likelihood function is simply the product of all marginal PDFs.  </strong>
</blockquote>

- Suppose $Y_{1},\ldots, Y_{n}$ are IID with PDF $f(y; \theta)$. Then:
  $$
  f(\mathbf{y}; \theta) = \prod_{i=1}^{n} f_{i}(y_i; \theta)
  $$

- Note that $f(\mathbf{y}; \theta)$ is the joint densities and $f_{i}(y_i; \theta)$ is for marginal densities; not the same functions

- In practice, one usually maximises the *log-likelihood* function, given by $l(\theta; \mathbf{y}) \equiv \ln L (\theta; \mathbf{y})$: $$\hat{\theta} = \arg \max_{\theta}l(\theta; \mathbf{y})$$.
- log is a monotonic transformation, $\ln L (\theta; \mathbf{y})$ and $L (\theta; \mathbf{y})$ are maximised at the same $\theta$. 
- log-likelihood is usually easier to deal with in mathematical proofs in numerical optimastion

- For simple likelihoods, usually for iid data, the ML estimator is available in closed form. 
...
## Marble Game for a discrete case
...
- When possible, having closed-form solutions is helpful, since it saves a lot of computation time: solving first order conditions numerically can be time-consuming.

### Post-Class Discussion{.fragment} 
